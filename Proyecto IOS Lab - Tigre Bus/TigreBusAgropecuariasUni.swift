//
//  TigreBusAgropecuariasUni.swift
...
class TigreBusAgropecuariasUni: UIViewController {
    
    var ref: DatabaseReference! = Database.database().reference().child("Rutas").child("Ruta 7")
    ...
    @IBAction func mensaje1(_ sender: UIButton) {
        if self.ruta1 == true{
            alerta()
        }else{
            if contadorParada == 0{
